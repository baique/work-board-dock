...
export default function SignalDock(): React.JSX.Element {
  ...
  if (compact) {
    return (
      <div
        className="flex h-screen w-screen flex-col items-center justify-center gap-4 glass select-none"
  ...
  return (
    <div
      className="flex h-screen w-screen flex-col glass text-white select-none overflow-hidden"
    ...
    >
      ...
      <div className="grid grid-cols-4 gap-1 px-2 pt-2">
        {SIGNAL_ORDER.map((state) => (
          <div
            key={state}
            className="flex flex-col items-center gap-1 rounded-xl bg-white/[0.05] px-1 py-1.5 transition-colors hover:bg-white/[0.09]"
          >
        ...
        <AnimatePresence>
          {cards.map((card) => (
            <motion.div
              ...
              className={`pointer-events-auto rounded-xl border p-2 shadow-xl backdrop-blur-xl ${
                card.level === "alert"
